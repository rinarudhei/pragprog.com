...
func TestRun(t *testing.T) {
	testcases := []struct {
		...
		{name: "FilterExtensionNoMatch", root: "testdata",
			cfg:      config{ext: ".exe", size: 0, list: true},
			expected: ""},
	}
	...
	for _, tc := range testcases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if res != tc.expected {
				t.Errorf("Expect '%q', but got '%q' instead", tc.expected, res)
			}
		})
	}
}
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if len(lines) != expLogLines {
				t.Errorf("Expected '%d' loglines, but got '%d' loglines instead\n", expLogLines, len(lines))
			}

			res := buffer.String()
			if res != tc.expected {
				t.Errorf("Expected '%q', but got '%q' instead", tc.expected, res)
			}
			...
			if len(filesLeft) != tc.nNoDelete {
				t.Errorf("Expect %d files left, but got %d files left instead", tc.nNoDelete, len(filesLeft))
			}
		})
	}
